...
<div class="space-y-6" x-data="{ showDeleteModal: false }">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <button type="button" @click="showDeleteModal = true" class="mt-3 px-4 py-2 text-sm text-red-600">Удалить категорию</button>
    </div>
    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @click.self="showDeleteModal = false">
        <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6" @click.stop>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
                Удалить категорию «{{ $category->name }}»? Если она используется в дереве или товарах, система не позволит удалить запись.
            </p>
            <form method="POST" action="{{ route('admin.catalog.categories.destroy', $category) }}" class="flex justify-end gap-3">
                @csrf
                @method('DELETE')
