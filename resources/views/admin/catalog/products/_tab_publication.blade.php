<div class="space-y-8">
    <div>
        ...
        <div class="bg-yellow-50 border border-yellow-200 text-yellow-700 dark:bg-yellow-900/20 dark:border-yellow-700 dark:text-yellow-200 px-4 py-3 rounded-lg mb-4">
            <div class="flex items-start gap-3">
                <svg class="w-5 h-5 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                </svg>
                <div>
                    <p class="font-medium">Карточка требует доработки перед публикацией в каталоге.</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4" x-data="{ status: '{{ old('status', $product->status) }}' }">
            <label class="relative flex cursor-pointer rounded-lg border p-4 transition-colors"
                :class="status === 'active' ? 'border-green-500 bg-green-50 dark:bg-green-900/20 ring-2 ring-green-500/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">Активен</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Виден дистрибьюторам и клиентам.</p>
                    </div>
                </div>
            </label>

            <label class="relative flex cursor-pointer rounded-lg border p-4 transition-colors"
                :class="status === 'hidden' ? 'border-gray-500 bg-gray-50 dark:bg-gray-700/50 ring-2 ring-gray-500/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">Скрыт</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Не отображается в каталоге.</p>
                    </div>
                </div>
            </label>

            <label class="relative flex cursor-pointer rounded-lg border p-4 transition-colors"
                :class="status === 'draft' ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20 ring-2 ring-yellow-500/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">Черновик</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Не отображается до публикации.</p>
                    </div>
                </div>
            </label>
        </div>
    ...
    @include('catalog.product._catalog_marks_form', ['product' => $product])

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Дата публикации</label>
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Оставьте пустым для немедленной публикации при активации.</p>
        </div>
        ...
        <div x-data="{ checked: {{ old('show_in_catalog', $product->show_in_catalog) ? 'true' : 'false' }} }" class="flex items-center">
            <label class="flex cursor-pointer items-center select-none">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">Товар будет отображаться в общем каталоге платформы</p>
                </div>
            </label>
        </div>
    </div>
    ...
    <div>
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Если не выбран ни один регион — товар доступен везде.
        </p>
    ...
    <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
        <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Информация о карточке</h4>
        <dl class="grid grid-cols-2 gap-4 text-sm">
            <div>
                <dt class="text-gray-500 dark:text-gray-400">Производитель</dt>
                <dd class="font-medium text-gray-900 dark:text-white">{{ $product->manufacturerProfile?->short_name ?: $product->manufacturerProfile?->full_name ?: '—' }}</dd>
            </div>
            <div>
            ...
            <div>
                <dt class="text-gray-500 dark:text-gray-400">Базовая цена</dt>
                <dd class="font-medium text-gray-900 dark:text-white">{{ $product->base_price ? number_format((float) $product->base_price, 2, ',', ' ') . ' ₽' : '—' }} <span class="text-xs text-gray-500">(только просмотр)</span></dd>
            </div>
            <div>
                <dt class="text-gray-500 dark:text-gray-400">Доступный остаток</dt>
                <dd class="font-medium text-gray-900 dark:text-white">{{ $product->available_stock }} <span class="text-xs text-gray-500">(только просмотр)</span></dd>
            </div>
        </dl>
    </div>
</div>
