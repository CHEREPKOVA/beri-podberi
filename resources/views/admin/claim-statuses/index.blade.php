@extends('layouts.app')
@section('title', 'Статусы претензий')
@section('heading', 'Статусы претензий')
@section('content')
<div class="space-y-6" x-data="{ showDeleteModal: false, deleteFormAction: '', deleteMessage: '' }">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border">
        <div class="p-6 border-b flex flex-col sm:flex-row sm:justify-between gap-4">
            ...
            <a href="{{ route('admin.claim-statuses.create') }}" class="px-4 py-2 bg-[#c3242a] text-white rounded-lg text-sm">Добавить</a>
        </div>
        ...
        <tbody class="divide-y">@forelse($statuses as $status)<tr class="group hover:bg-gray-50 dark:hover:bg-gray-700/50">
            <td class="px-4 py-3 text-sm"><a href="{{ route('admin.claim-statuses.edit', $status) }}" class="font-medium hover:text-[#c3242a]">{{ $status->name }}</a></td>
