...
<div x-data="{ activeTab: '{{ $tab }}', showDeleteModal: false, deleteFormAction: '', deleteMessage: '' }" class="space-y-6">
                    ...
                    @forelse($employees as $employee)
                        <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                            ...
                            <div class="mt-3 flex flex-wrap gap-2">
                                ...
                                <button type="button"
                                    @click="deleteFormAction = '{{ route('admin.companies.users.delete', [$companyKey, $employee]) }}'; deleteMessage = {{ json_encode('Удалить пользователя «' . $employee->name . '» из компании?') }}; showDeleteModal = true"
                                    class="px-3 py-1.5 rounded-lg bg-red-100 text-red-700 text-xs">
                                    Удалить
                                </button>
                            </div>
                        </div>
    ...
    <div x-show="showDeleteModal" x-cloak
        ...
        <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6" @click.stop>
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Удаление пользователя</h3>
