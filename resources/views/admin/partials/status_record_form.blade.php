...
<div>
    ...
    <textarea name="description" id="description" rows="3"
        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700">{{ old('description', $item->description ?? '') }}</textarea>
...
<div>
    ...
    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $item->sort_order ?? 0) }}" min="0" required
...
<div class="flex items-center gap-2">
    ...
    <input type="checkbox" name="is_terminal" id="is_terminal" value="1" @checked(old('is_terminal', $item->is_terminal ?? false)) class="h-4 w-4 rounded accent-[#c3242a]" />
    <label for="is_terminal" class="text-sm text-gray-700 dark:text-gray-300 cursor-pointer">Финальный статус</label>
</div>
...
<div class="flex items-center gap-2">
    ...
    <label for="is_active" class="text-sm text-gray-700 dark:text-gray-300 cursor-pointer">Активен</label>
</div>
