...
@section('heading', 'Добавить сотрудника')

@section('content')
<div class="max-w-7xl">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <form method="POST" action="{{ route('admin.staff.store') }}" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            @csrf
            ...
            <div class="space-y-5 lg:col-span-5">
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Пароль <span class="text-red-500">*</span></label>
            ...
            <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/40 p-4 space-y-4 lg:col-span-7">
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Можно переопределить права роли для этого сотрудника. По умолчанию действует режим «Наследовать».</p>
                </div>
                    ...
                    <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3">
                        ...
                        <div class="mt-3 flex flex-wrap items-center gap-4 text-sm">
                            <label class="inline-flex items-center gap-2">
                                <input type="radio" name="permission_overrides[{{ $permission->id }}]" value="inherit" class="h-4 w-4 border-gray-300 accent-[#c3242a] focus:ring-[#c3242a]" {{ old('permission_overrides.'.$permission->id, 'inherit') === 'inherit' ? 'checked' : '' }}>
                                <span>Наследовать</span>
                            </label>
                            <label class="inline-flex items-center gap-2">
                                <input type="radio" name="permission_overrides[{{ $permission->id }}]" value="allow" class="h-4 w-4 border-gray-300 accent-[#c3242a] focus:ring-[#c3242a]" {{ old('permission_overrides.'.$permission->id) === 'allow' ? 'checked' : '' }}>
                                <span>Разрешить</span>
                            </label>
                            <label class="inline-flex items-center gap-2">
                                <input type="radio" name="permission_overrides[{{ $permission->id }}]" value="deny" class="h-4 w-4 border-gray-300 accent-[#c3242a] focus:ring-[#c3242a]" {{ old('permission_overrides.'.$permission->id) === 'deny' ? 'checked' : '' }}>
                                <span>Запретить</span>
                            </label>
                        </div>
                    </div>
