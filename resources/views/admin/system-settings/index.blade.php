        ...
        @forelse($settings as $groupKey => $groupSettings)
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 class="font-semibold text-gray-900 dark:text-white">{{ \App\Models\SystemSetting::groupLabel($groupKey) }}</h2>
                </div>
                                ...
                                @if($setting->value_type === 'boolean')
                                    ...
                                    <label class="inline-flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="settings[{{ $setting->id }}][value]" value="1" @checked((string) $setting->value === '1')
                                            onchange="this.nextElementSibling.textContent = this.checked ? 'Включено' : 'Выключено'"
                                        ...
                                        <span class="text-sm text-gray-700 dark:text-gray-300">{{ (string) $setting->value === '1' ? 'Включено' : 'Выключено' }}</span>
                                    </label>
