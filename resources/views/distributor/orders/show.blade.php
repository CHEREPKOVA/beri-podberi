        ...
        ->map(function ($row) use ($catalogProductOptions) {
            ...
            return [
                ...
                'unit_price' => isset($row['unit_price']) && $row['unit_price'] !== ''
                    ...
                    : $product['price'],
                'warehouse_id' => isset($row['distributor_warehouse_id']) && $row['distributor_warehouse_id'] !== ''
                    ? (string) $row['distributor_warehouse_id']
                    : '',
            ];
        })
        ->filter()
...
<div class="space-y-6" x-data="{
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <a href="{{ route('distributor.orders.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-[#c3242a]">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            К моим заказам
        </a>
        <a href="{{ route('distributor.orders.print', $order) }}" target="_blank"
        ...
        <section class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
                @if(in_array(OrderStatusWorkflowService::ACTION_CONFIRM, $actions, true))
                    <form method="POST" action="{{ route('distributor.orders.confirm', $order) }}">
                        @csrf
                ...
                @if(in_array(OrderStatusWorkflowService::ACTION_REJECT, $actions, true))
                    <button type="button" @click="rejectOpen = true; shipOpen = false; editOpen = false" class="{{ $btnOutline }}">Отклонить</button>
                @endif

                @if(in_array(OrderStatusWorkflowService::ACTION_SEND_FOR_APPROVAL, $actions, true))
                    <button type="button" @click="editOpen = true; rejectOpen = false; shipOpen = false" class="{{ $btnOutline }}">Изменить заказ</button>
                @endif

                @if(in_array(OrderStatusWorkflowService::ACTION_MARK_IN_WORK, $actions, true))
                    <form method="POST" action="{{ route('distributor.orders.mark_in_work', $order) }}">
                        @csrf
                ...
                @if(in_array(OrderStatusWorkflowService::ACTION_MARK_READY, $actions, true))
                    <form method="POST" action="{{ route('distributor.orders.mark_ready', $order) }}">
                        @csrf
                ...
                @if(in_array(OrderStatusWorkflowService::ACTION_MARK_SHIPPED, $actions, true))
                    <button type="button" @click="shipOpen = true; rejectOpen = false; editOpen = false" class="{{ $btnPrimary }}">Отметить как отгружен</button>
                @endif
            </div>

            <div x-show="rejectOpen" x-cloak class="border-2 border-[#c3242a]/30 rounded-lg p-4 bg-red-50/40 dark:bg-red-900/10">
                <form method="POST" action="{{ route('distributor.orders.reject', $order) }}" class="space-y-3">
                    @csrf
            ...
            <div x-show="shipOpen" x-cloak class="border-2 border-[#c3242a]/30 rounded-lg p-4 bg-red-50/40 dark:bg-red-900/10">
                <form method="POST" action="{{ route('distributor.orders.mark_shipped', $order) }}" class="space-y-3">
                    @csrf
                    <div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="shipping_from_warehouse" class="{{ $inputBrand }} appearance-none pr-10 cursor-pointer">
                                <option value="">— не указан —</option>
                                @foreach($warehouses as $warehouse)
                                    <option value="{{ $warehouse->name }}"
                                        @selected(old('shipping_from_warehouse', $order->shipping_from_warehouse) === $warehouse->name)>
                                        {{ $warehouse->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="transport_company_id" class="{{ $inputBrand }} appearance-none pr-10 cursor-pointer">
            ...
            <div id="edit" x-show="editOpen" x-cloak class="border-2 border-[#c3242a]/30 rounded-lg p-4 bg-red-50/40 dark:bg-red-900/10">
                <form method="POST" action="{{ route('distributor.orders.send_for_approval', $order) }}" class="space-y-3">
                    @csrf
                    <p class="text-sm text-gray-600 dark:text-gray-300">
                        Измените количество, цену или склад, удалите позиции или добавьте товары из своей номенклатуры — затем отправьте на согласование покупателю. После отправки заказ перейдёт в статус «Требует согласования».
                    </p>
                    <div class="overflow-x-auto">
                        <table class="w-full min-w-[56rem] text-sm">
                            <thead>
                                <tr class="text-xs text-gray-500 dark:text-gray-400">
                                    <th class="text-left font-medium pb-2 pr-3">Товар</th>
                                    <th class="text-left font-medium pb-2 px-2 w-24">Кол-во</th>
                                    <th class="text-left font-medium pb-2 px-2 w-32">Цена</th>
                                    <th class="text-left font-medium pb-2 px-2 w-48">Склад</th>
                                    <th class="text-left font-medium pb-2 px-2 w-44">Причина</th>
                                    <th class="text-left font-medium pb-2 pl-2 w-28"></th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr class="border-b border-gray-100 dark:border-gray-700 align-top">
                                        ...
                                        <td class="py-3 px-2 w-24">
                                            <input type="number" min="1" name="items[{{ $index }}][quantity]" value="{{ old('items.'.$index.'.quantity', $item->quantity) }}"
                                                   class="{{ $inputNumber }}" aria-label="Количество">
                                        </td>
                                        <td class="py-3 px-2 w-32">
                                            <input type="number" min="0" step="0.01" name="items[{{ $index }}][unit_price]" value="{{ old('items.'.$index.'.unit_price', $item->unit_price) }}"
                                                   class="{{ $inputNumber }}" aria-label="Цена">
                                        </td>
                                        <td class="py-3 px-2 w-48">
                                            <div class="relative">
                                                <select name="items[{{ $index }}][distributor_warehouse_id]" class="{{ $selectBrand }}" aria-label="Склад">
                                                    <option value="">— не указан —</option>
                                                    @foreach($warehouses as $warehouse)
                                                        <option value="{{ $warehouse->id }}" @selected((string) old('items.'.$index.'.distributor_warehouse_id', $item->distributor_warehouse_id) === (string) $warehouse->id)>
                                                            {{ $warehouse->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <td class="py-3 px-2 w-44">
                                            <input type="text" name="items[{{ $index }}][reason]" value="{{ old('items.'.$index.'.reason') }}"
                                                   class="{{ $inputBrand }}" placeholder="Необязательно" aria-label="Причина">
                                        </td>
                                        <td class="py-3 pl-2 w-28">
                                            <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none h-10">
                                ...
                                <template x-for="(row, idx) in extraItems" :key="row.id">
                                    <tr class="border-b border-dashed border-gray-200 dark:border-gray-600 align-top">
                                        <td class="py-3 pr-3 min-w-0">
                                            <input type="hidden" :name="'items[' + ({{ $itemCount }} + idx) + '][distributor_product_id]'" :value="row.id">
                                            <p class="font-medium text-gray-900 dark:text-white break-words" x-text="row.name"></p>
                                            <p class="text-xs text-gray-500 mt-0.5" x-text="row.sku || '—'"></p>
                                        </td>
                                        <td class="py-3 px-2 w-24">
                                            <input type="number" min="1" :name="'items[' + ({{ $itemCount }} + idx) + '][quantity]'" :value="row.quantity" class="{{ $inputNumber }}" aria-label="Количество">
                                        </td>
                                        <td class="py-3 px-2 w-32">
                                            <input type="number" min="0" step="0.01" :name="'items[' + ({{ $itemCount }} + idx) + '][unit_price]'" :value="row.unit_price" class="{{ $inputNumber }}" aria-label="Цена">
                                        </td>
                                        <td class="py-3 px-2 w-48">
                                            <div class="relative">
                                                <select :name="'items[' + ({{ $itemCount }} + idx) + '][distributor_warehouse_id]'" x-model="row.warehouse_id" class="{{ $selectBrand }}" aria-label="Склад">
                                                    <option value="">— не указан —</option>
                                                    @foreach($warehouses as $warehouse)
                                        ...
                                        <td class="py-3 px-2 w-44"></td>
                                        <td class="py-3 pl-2 w-28">
                                            <button type="button" @click="removeExtra(row.id)" class="{{ $btnGhost }} h-10">Убрать</button>
                                        </td>
                                    </tr>
                                </template>
                            </tbody>
                    ...
                    <p x-show="catalog.length === 0" x-cloak class="text-xs text-gray-500">В номенклатуре нет доступных товаров.</p>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            <section class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 space-y-4 text-sm">
                ...
                <p>
                    <span class="text-gray-500">Источник заказа:</span>
                    <span class="font-medium text-gray-900 dark:text-white">{{ $order->sourceLabel() }}</span>
                </p>
                <form method="POST" action="{{ route('distributor.orders.assign_responsible', $order) }}" class="space-y-3">
                    @csrf
                    <div>
                        ...
                        <div class="relative">
                            <select name="responsible_contact_id" class="{{ $inputBrand }} appearance-none pr-10 cursor-pointer">
                                ...
                                @foreach($managers as $manager)
                                    <option value="{{ $manager->id }}" @selected((string) old('responsible_contact_id', $order->responsible_contact_id) === (string) $manager->id)>
