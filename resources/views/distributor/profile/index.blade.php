...
@section('title', 'Профиль дистрибьютора')
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div class="p-6">
            ...
            <div x-show="activeTab === 'company'" x-cloak>
                @include('distributor.profile._company', ['profile' => $profile])
            </div>

            {{-- Таб: Контактные данные --}}
            <div x-show="activeTab === 'contacts'" x-cloak>
                @include('distributor.profile._contacts', ['profile' => $profile])
            </div>
            ...
            <div x-show="activeTab === 'delivery'" x-cloak>
                @include('distributor.profile._delivery', ['profile' => $profile, 'deliveryMethods' => $deliveryMethods, 'transportCompanies' => $transportCompanies])
            </div>

            {{-- Таб: Интеграция --}}
            <div x-show="activeTab === 'integration'" x-cloak>
                @include('distributor.profile._integration', ['profile' => $profile])
            </div>

            {{-- Таб: Документы --}}
            <div x-show="activeTab === 'documents'" x-cloak>
                @include('distributor.profile._documents', ['profile' => $profile])
            </div>
        </div>
    </div>
</div>
