...
<div class="space-y-6">
    ...
    <a href="{{ route('manufacturer.catalog.analogs.index') }}" class="text-sm text-gray-500 hover:text-[#c3242a]">← Аналоги</a>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <p class="text-sm text-gray-500 mt-1">Выберите официальные взаимозаменяемые позиции. Связи сохраняются двусторонне.</p>

        <form method="POST" action="{{ route('manufacturer.catalog.analogs.update', $product) }}" class="mt-6">
            @csrf
            @method('PUT')
