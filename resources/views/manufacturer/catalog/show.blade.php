...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <a href="{{ $product->category?->slug ? route('manufacturer.catalog.index', $product->category->slug) : route('manufacturer.catalog.index') }}"
                ...
                @foreach($analogs as $analog)
                    <div class="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                        ...
                        <div class="p-3">
                            ...
                            @if($analog->manufacturer_profile_id === $product->manufacturer_profile_id)
                                <a href="{{ route('manufacturer.catalog.show', $analog) }}" class="inline-flex mt-2 text-sm text-[#c3242a] hover:text-[#a01e24]">Открыть карточку</a>
                            @else
                                <span class="inline-flex mt-2 text-sm text-gray-400">Недоступно в вашем каталоге</span>
                            @endif
                        </div>
                    </div>
                @endforeach
